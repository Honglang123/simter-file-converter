...
class AppConfiguration @Autowired constructor(
  @Value("\${simter-file-converter.rest-context-path}") private val contextPath: String,
  @Value("\${simter-file-converter.from-root-dir}") private val fromRootDir: String,
  @Value("\${simter-file-converter.to-root-dir}") private val toRootDir: String,
  ...
  @Value("\${simter-file-converter.version:UNKNOWN}") private val simterFileVersion: String,
  @Value("\${simter-file-converter.dependency-version.simter:UNKNOWN}") private val simterVersion: String,
  @Value("\${simter-file-converter.dependency-version.kotlin:UNKNOWN}") private val kotlinVersion: String,
  @Value("\${simter-file-converter.dependency-version.spring-framework:UNKNOWN}") private val springFrameworkVersion: String,
  @Value("\${simter-file-converter.dependency-version.spring-boot:UNKNOWN}") private val springBootVersion: String
) {
  private val logger = LoggerFactory.getLogger(AppConfiguration::class.java)

  init {
    if (logger.isInfoEnabled) {
      logger.info("simter-file-converter.rest-context-path={}", contextPath)
      logger.info("simter-file-converter.from-root-dir={}", fromRootDir)
      logger.info("simter-file-converter.to-root-dir={}", toRootDir)
  ...
  private val rootPage: String = """
    ...
    <head>
      ...
      <title>simter-file-converter</title>
      <style>html{background-color:#000;color:#fff}</style>
    </head>
    <body>
      <h2>Simter File Converter Service</h2>
  ...
  fun rootRoutes() = router {
    "/".nest {
      // root /
      GET("/") { ok().contentType(MediaType.TEXT_HTML).bodyValue(rootPage) }
      // '/favicon.ico'
      GET("/favicon.ico") {
  ...
  fun corsFilter4StaticFile(): WebFilter {
    return WebFilter { exchange: ServerWebExchange, chain: WebFilterChain ->
      ...
      if (CorsUtils.isCorsRequest(request)                   // cross origin
        ...
        && request.path.value().startsWith("/static/")
      ) {    // only for static file dir
